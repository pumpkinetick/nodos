...
class BorderLayer(RenderLayer):
    ...
    def build(self, world_map):
        ...
        for hex_obj, tile in world_map.tiles.items():
            if tile.city_id is not None:
                ...
                b_shapes = self._bake_tile_borders(hex_obj=hex_obj, corners=corners, tile=tile, world_map=world_map)
                if b_shapes:
    ...
    def _bake_tile_borders(self, hex_obj, corners, tile, world_map):
        city = world_map.cities[tile.city_id]
        border_list = []
        for i, (dq, dr) in enumerate(POINTY_TOP_DIRECTIONS):
            neighbor = world_map.get_tile(hex_obj=HexObject(q=hex_obj.q + dq, r=hex_obj.r + dr))
